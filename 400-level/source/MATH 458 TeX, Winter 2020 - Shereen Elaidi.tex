...
\begin{definition}[One-Parameter Family of (Straight) Lines]
	A \dfn{differentiable} \dfn{one-parameter} \dfn{family} \dfn{of} \dfn{(straight) lines} $\{ \alpha(t), w(t) \}$ is a correspondence that assigns to each $t \in I$ a point $\alpha(t) \in \R^3$ and a vector $w(t) \in \R^3$ so that both $\alpha(t)$ and $w(t)$ depend differentiably on $t$.
\end{definition}

\begin{definition}[Ruled Surface]
	Given a one-parameter family of lines $\{ \alpha(t), w(t) \}$, the parametrised surface: 
	\begin{align}
		x(t,v) = \alpha (t) + v w(t),\ t \in I,\ v \in \R 	
	\end{align}
	is called the \dfn{ruled surface} generated by the family $\{ \alpha(t), w(t) \}$. 
	\begin{enumerate}[noitemsep]
		\item The lines $L_t$ are called the \dfn{rulings}: for each $t \in I$, the line $L_t$ which passes through $\alpha(t)$ and is parallel to $w(t)$. 
		\item The curve $\alpha(t)$ is called a \dfn{directrix} 
	\end{enumerate}
\end{definition}
\subsubsection{Minimal Surface}
\begin{definition}[Minimal Surface]
	A regular parameterised surface is called \dfn{minimal} if its mean curvature vanishes everywhere. A regular surface $S \subseteq \R^3$ is called \dfn{minimal} if each of its parameterisations is minimal. 
\end{definition}
